def budget_tool(budget_min: int, budget_max: int, segments: list):
    """Analyze and validate budget allocation against segments"""
    ...
    if fit == 'under':
        ...
        suggestions.append(f"You have Rs.{remaining} remaining. Consider upgrading the dining experience.")
        suggestions.append("You could add a dessert stop at a premium patisserie.")
    elif fit == 'over':
        excess = total - budget_max
        suggestions.append(f"Budget exceeded by Rs.{excess}. Consider these adjustments:")
